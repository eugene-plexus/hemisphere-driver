...
(`EUGENE_PLEXUS_HD_AUTH_SIGNING_KEY`). This module exposes just the
decode side so route dependencies can validate inbound bearer tokens.

Mirror of `eugene_plexus_orchestrator.security`; keeping these files
nearly identical across components makes the cross-cutting contract
